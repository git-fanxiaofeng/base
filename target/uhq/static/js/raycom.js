...
$(document).ready(function () {
	...
	$('body').on('click', '.show-sidebar', function (e) {
		e.preventDefault();
		//$('div#main',window.parent.document).toggleClass('sidebar-show contet-width');
...
function actCancel(oform, action, target) {
	...
	//弹出框
function raycomShowApplylog(applyMasterId){
	var url="/pierceQuery/queryApplyDetail?applyMasterId="+applyMasterId;
	//模态窗设置
	var iWidth=1200; //窗口宽度 
	var iHeight=500;//窗口高度 
	var iTop=(window.screen.height-iHeight)/2; 
	var iLeft=(window.screen.width-iWidth)/2; 
	window.showModalDialog(ctx+url,null,"dialogHeight= "+iHeight+"px; dialogWidth= "+iWidth+"px; dialogTop= "+iTop+"px; dialogLeft= "+iLeft+"px; resizable= no; status= no;scroll=no");
}
